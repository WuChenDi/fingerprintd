...
the collect-only [`@fingerprintd/client`](../../packages/client) SDK against a
...
- **Collected evidence** — the four lanes straight out of the SDK's `Collected`,
  each in its own section:
  - `stable_components` — the "who is this device" matching input.
  - `challenge_response` — nonce-seeded freshness proof (never a matching signal).
  - `probe` — `hex(HMAC-SHA256(key, nonce))` computed in WASM (sent only when the
    challenge advertises `collect.challenge.verify`).
  - `ts` — the client clock at collection.
- **Response signature (T9)** — whether the server signed the response; supply a
...
bun run --filter @fingerprintd/client build
...
```bash
bun run --filter @fingerprintd/client build   # SDK dist (gitignored)
